...
from strategies.model_1 import _interpolate_spread, compute_levels
...
class TestComputeLevels:
    ...
    def test_returns_order_levels_for_normal_market(self):
        with self._patch():
            levels = compute_levels(self._market())
        assert levels is not None
        assert isinstance(levels, OrderLevels)

    def test_buy_yes_below_mid(self):
        with self._patch():
            levels = compute_levels(self._market(0.50, 0.50))
        assert levels is not None
    ...
    def test_extreme_price_returns_none(self):
        with self._patch():
            levels = compute_levels(self._market(0.03, 0.97))
    ...
    def test_higher_vol_wider_spread(self):
        with patch.multiple(
            ...
            with patch("strategies.model_1.DEFAULT_VOLATILITY", 0.005):
                levels_low = compute_levels(self._market())
            with patch("strategies.model_1.DEFAULT_VOLATILITY", 0.025):
                levels_high = compute_levels(self._market())
